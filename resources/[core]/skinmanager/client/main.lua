...
function LoadDefaultModel(malePed, cb)
    ...
    CreateThread(function()
        ...
        TriggerEvent('skinmanager:client:modelLoaded')
    end)
end
...
AddEventHandler('skinmanager:client:loadDefaultModel', function(loadMale, cb)
    LoadDefaultModel(loadMale, cb)
end)

AddEventHandler('skinmanager:client:getData', function(cb)
...
AddEventHandler('skinmanager:client:change', function(key, val)
    Character[key] = val

    if key == 'sex' then
        TriggerEvent('skinmanager:client:loadSkin', Character)
    else
        ApplySkin(Character)
    end
end)

AddEventHandler('skinmanager:client:getSkin', function(cb)
    cb(Character)
end)

AddEventHandler('skinmanager:client:modelLoaded', function()
...
RegisterNetEvent('skinmanager:client:loadSkin', function(skin, cb)
    if skin['sex'] ~= LastSex then
        LoadSkin = skin

        if skin['sex'] == 0 then
            TriggerEvent('skinmanager:client:loadDefaultModel', true, cb)
        else
            TriggerEvent('skinmanager:client:loadDefaultModel', false, cb)
        end
    else
        ApplySkin(skin)
...
RegisterNetEvent('skinmanager:client:loadClothes', function(playerSkin, clothesSkin)
    if playerSkin['sex'] ~= LastSex then
        ...
        if playerSkin['sex'] == 0 then
            TriggerEvent('skinmanager:client:loadDefaultModel', true)
        else
            TriggerEvent('skinmanager:client:loadDefaultModel', false)
        end
    else
